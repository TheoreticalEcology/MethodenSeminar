...
#standard deviations of the group-level coefficient
sd_groups <- c(3, 15, 0.7, 1.2)
...
Beta_groups <- mapply(function(c, sd) rnorm(n_groups, c, sd), c = beta_pop, sd = sd_groups)
...
fit_lmer <- lme4::lmer(y ~ X[,2] + X[,3] + X[,4] + ( 1 + X[,2] + X[,3] + X[,4] | group),
           data = data)
summary(fit_lmer)
fit_stan <- stan('Bayesian/Hierarchical.stan',
                 data = data,
                 init = 'random',
                 warmup = 3000, # if not specified: automatically half of the iterations
                 iter = 10000,
