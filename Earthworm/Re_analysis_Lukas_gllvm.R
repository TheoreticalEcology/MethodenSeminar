# setwd("~/Documents/Studium/Seminar Methods/Earthworm") ## source file location
...
# 3. ranger
learner = makeLearner("regr.ranger", importance = "impurity")
getParamSet(learner) # mse.test.mean=15059.5396512

result = resample(learner = learner, task = task, resampling = cv10, measures = mlr::mse, models = )
...
# Lol, laeuft nicht ---------------------------------------------------------------------
library(gllvm) # Analysis of multivariate data using generalized linear latent variable models (gllvm). Estimation is performed using either Laplace approximation method or variational approxima- tion method implemented via TMB (Kristensen et al., (2016), <doi:10.18637/jss.v070.i05>).


Data <- abundance %>% select(logAbundance, elevation, scalePH, 
                          CLYPPT, SLTPPT, CECSOL, ORCDRC,bio10_7,
                          bio10_15, Aridity, PETyr,
                          logBiomass, SpeciesRichness) %>%
  na.omit()


Data$logAbundance = exp(Data$logAbundance) - 1 
Data$logBiomass = exp(Data$logBiomass) - 1 

Resp <- as.matrix(Data[, c('logAbundance', 'logBiomass', 'SpeciesRichness')])
Pred <- Data[, !(names(Data) %in% c('logAbundance', 'logBiomass', 'SpeciesRichness'))] %>%
  apply(MARGIN = 2, as.numeric) %>%
  as.data.frame()

m <- gllvm(y = Resp, X = Pred, family = 'gaussian', starting.val = 'zero')
# does not converge :D
